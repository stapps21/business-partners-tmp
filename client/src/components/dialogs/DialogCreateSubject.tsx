...
const DialogCreateSubject = () => {
    ...
    const { handleSubmit: handleCreate } = useCreateData<RequestReferenceData>('/subjects', 'subjects');

    const onSubmit = (data: RequestReferenceData) => {
        handleCreate(data, {
            onSuccess: () => setIsDialogOpen(false),
            onError: (error: Error) => console.error('Error creating subject:', error),
        });
    };
    ...
    return (
        <>
            ...
            {
                isDialogOpen &&
                <CustomDialog open={isDialogOpen} onClose={() => setIsDialogOpen(false)} title='Add new subject' labelPositiveButton='Add subject' disabled={isSubmitting} onConfirm={handleSubmit(onSubmit)}>
                    <DialogContent>
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <DialogContent>
                                <CustomSingleDialogTextField name="name" label="Subject" control={control} />
                            </DialogContent>
                        </form>
                    </DialogContent>
                </CustomDialog>
...
export default DialogCreateSubject;
